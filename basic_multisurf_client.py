...
class MultiSurfClient:
	...
	def sendRequest(h, p):
		...
		myConn.putheader('User-Agent', user_agent_hdr)
		myConn.putheader('Cookie', cookie_hdr)
		myConn.endheaders()
	...
	def sendPeerReq(ip, port, rawUrl):
		...
		sslSocket.send(len(user_agent))
		sslSocket.send(user_agent))
		sslSocket.send(len(cookies))
		sslSocket.send(cookies))
